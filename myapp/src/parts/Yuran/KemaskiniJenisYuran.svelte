<script>
  ...
  const submitHandler = async (e) => {
    ...
    if (e.target.getAttribute("id") == "tambahKadarYuran") {
      ...
      submitpost = JSON.parse(await apidata).submitpost;
      dispatch("menuChange", ["Jenis Yuran", "Berjaya Kemaskini Jenis Yuran"]);
      return;
    }
  };
  ...
  const editKadar = (e) => {
    ...
    if (dataid == null) {
      dataid = e.target.parentElement.getAttribute("data-pass");
    }
    if (dataid == null) {
      dataid = e.target.parentElement.parentElement.getAttribute("data-pass");
    }
...
<main>
  <div class="container-fluid px-4">
    <h1 class="mt-4">Kemaskini Jenis Yuran</h1>
    ...
    {:else}
      ...
      <div>
        <Modal size="lg" isOpen={open} {toggle}>
          <form id="tambahKadarYuran" on:submit|preventDefault={submitHandler}>
