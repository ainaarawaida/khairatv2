...
const submitHandler = async () => {
      ...
      let apidata = new Promise(function (myResolve, myReject) {
        ...
        fields.kariah_id =  (submitpost.kariah_id == undefined) ? '' : submitpost.kariah_id ;
        ...
        <div class="container px-5 my-5">
            <form id="contactForm" on:submit|preventDefault={submitHandler}>
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control {submitpost.error.findIndex(p => p.hasOwnProperty('namaKariah')) != -1
                    ? 'is-invalid'
                    : ''}" id="namaKariah" type="text" placeholder="Nama Kariah" required data-sb-validations="required" bind:value={fields.namaKariah}/>
                    <div class="invalid-feedback" data-sb-feedback="namaKariah:required">Nama Kariah is required.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="alamatKariah">Alamat Kariah</label>
                    <textarea class="form-control {submitpost.error.findIndex(p => p.hasOwnProperty('alamatKariah')) != -1
                        ? 'is-invalid'
                        : ''}" id="alamatKariah" type="text" placeholder="Alamat Kariah" style="height: 10rem;" data-sb-validations="required" required bind:value={fields.alamatKariah} />
                    <div class="invalid-feedback" data-sb-feedback="alamatKariah:required">Alamat Kariah is required.</div>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="sitePageUrl">Site Page URL</label>
                    <input class="form-control {submitpost.error.findIndex(p => p.hasOwnProperty('sitePageUrl')) != -1
                    ? 'is-invalid'
                    : ''}" id="sitePageUrl" type="text" placeholder="Site Page URL" data-sb-validations="required" bind:value={fields.sitePageUrl} required/>
                    <div class="invalid-feedback" data-sb-feedback="sitePageUrl:required">Site Page URL is required.</div>
                </div>
